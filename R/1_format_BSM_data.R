...
formatData <- function() {
  ...
  GDD_lakes <- utils::read.csv(
    "data/bsm_clim_all_yrs_hwy_ap.csv",
    stringsAsFactors = FALSE
  ) |> dplyr::select(-X)
  saveRDS(GDD_lakes, file = "data/bsm_lake_clim.rds")
  # BSM DATA
  BSM1 <- utils::read.csv("data/bsm_cy1_aged.csv", header = TRUE, sep = ",", comment.char = "", fill = TRUE)
  ...
  ## read cycle 2
  BSM2 <- utils::read.csv("data/bsm_cy2_aged.csv", header = TRUE, sep = ",", comment.char = "", fill = TRUE)
  ...
  ## -- Cycle 2
  BSM2_Lakes <- utils::read.csv("data/bsm_cy2_lake_data.csv",
    header = TRUE,
    sep = ","
  ) |>
